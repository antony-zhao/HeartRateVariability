...
def random_sampling(ecg, filtered_ecg, signal, samples, ensure_labels=False):
    ...
    x, y = [], []
    pad = stack // 2
    padded_ecg = np.pad(ecg, (int(pad * window_size), int(pad * window_size)), constant_values=(0, 0))
    padded_filter = np.pad(filtered_ecg, (int(pad * window_size), int(pad * window_size)), constant_values=(0, 0))
    indices = np.random.randint(0, len(ecg) - int(pad * window_size), size=samples)
    for ind in indices:
