...
def process_ecg(ecg, filtered_ecg, scale_down, stack, datapoints):
    ...
    return ecg.reshape((4 * stack, datapoints))
...
#             if x_train is None:
#                 x_train, y_train = random_sampling_masking(ecg, filtered_ecg1, sig1, samples)
#             else:
#                 temp1, temp2 = random_sampling_masking(ecg, filtered_ecg1, sig1, samples)
#                 if temp1.size == 0:
#                     continue
#                 x_train = np.append(x_train, temp1, axis=0)
#                 y_train = np.append(y_train, temp2, axis=0)
...
#         if x_test is None:
#             x_test, y_test = random_sampling_masking(ecg2, filtered_ecg2, sig2, samples)
#         else:
#             if len(ecg2) < lines // 2:
#                 break
#             temp1, temp2 = random_sampling_masking(ecg2, filtered_ecg2, sig2, samples)
#             if len(temp1) > 0:
#                 x_test = np.append(x_test, temp1, axis=0)
#                 y_test = np.append(y_test, temp2, axis=0)
#
